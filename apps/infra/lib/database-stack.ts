/**
 * DatabaseStack — Cognito, DynamoDB, S3/CloudFront hosting, monitoring.
 */
...
export class DatabaseStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    this.userPoolClient = this.userPool.addClient('WebClient', {
      userPoolClientName: 'ProjectPlanner-WebClient',
      authFlows: { userPassword: true, userSrp: true },
      preventUserExistenceErrors: true,
    })

    this.identityPool = new cognito.CfnIdentityPool(this, 'IdentityPool', {
      identityPoolName: 'ProjectPlanner-IdentityPool',
    ...
    const oac = new cloudfront.S3OriginAccessControl(this, 'OAC', { description: 'OAC for Project Planner AI frontend' })
    ...
    // ── Monitoring ───────────────────────────────────────────────────────────
    this.alarmTopic = new sns.Topic(this, 'AlarmTopic', { topicName: 'ProjectPlanner-Alarms' })
    ...
    this.plansTable.metric('SystemErrors', { period: cdk.Duration.minutes(1) })
      ...
      .addAlarmAction(new cwActions.SnsAction(this.alarmTopic))
