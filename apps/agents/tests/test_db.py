...
TABLE_NAME = app_config.planner_plans_table
...
class TestGetPlan:
    def test_returns_item_when_exists(self, dynamodb_env):
        db_mod, table = dynamodb_env
        table.put_item(Item={"planId": "plan-1", "status": "DRAFT"})
        result = db_mod.get_plan("plan-1")
    ...
    def test_returns_none_when_not_found(self, dynamodb_env):
        db_mod, table = dynamodb_env
        result = db_mod.get_plan("nonexistent")
...
class TestPutItem:
    def test_puts_and_retrieves_item(self, dynamodb_env):
        db_mod, table = dynamodb_env
        db_mod.put_item(TABLE_NAME, {"planId": "plan-2", "data": "hello"})
        resp = table.get_item(Key={"planId": "plan-2"})
...
class TestUpdateItem:
    def test_updates_single_field(self, dynamodb_env):
        db_mod, table = dynamodb_env
        table.put_item(Item={"planId": "plan-3", "status": "DRAFT"})
        ...
        assert result["planId"] == "plan-3"

    def test_updates_multiple_fields(self, dynamodb_env):
        db_mod, table = dynamodb_env
        table.put_item(Item={"planId": "plan-4", "status": "DRAFT", "output": ""})
        result = db_mod.update_item(
            TABLE_NAME,
            {"planId": "plan-4"},
    ...
    def test_adds_new_field_via_update(self, dynamodb_env):
        db_mod, table = dynamodb_env
        table.put_item(Item={"planId": "plan-5"})
        result = db_mod.update_item(
            TABLE_NAME,
            {"planId": "plan-5"},
            {"newField": "value"},
