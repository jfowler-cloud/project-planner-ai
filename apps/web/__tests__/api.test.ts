...
vi.mock('@/config/amplify', () => ({
  ...
  appConfig: { plansTable: 'test-table', handoffTable: 'test-handoff-table', workflowArn: 'arn:aws:states:us-east-1:123:stateMachine:test' },
  scaffoldConfig: { url: 'http://localhost:3001' },
}));
...
describe('exportToScaffold', () => {
  ...
  it('writes plan data to DynamoDB handoff table and returns sessionId', async () => {
    mockSend.mockResolvedValueOnce({});
  ...
  it('throws on DynamoDB failure', async () => {
    ...
    await expect(exportToScaffold({})).rejects.toThrow('DynamoDB write failed');
  });
});
